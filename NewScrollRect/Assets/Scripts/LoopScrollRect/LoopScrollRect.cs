...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        public void SetScrollRect(int Count, Type UIType, string itemName)
        {
            totalCount = Count;
            m_uiType = UIType;
            prefabSource.prefabName = itemName;
            prefabSource.poolSize = 5;
            RefillCells();
        }
        ...
        public void RefillCells(int offset = 0)
        {
            ...
            while(sizeToFill > sizeFilled)
            {
                float size = reverseDirection ? NewItemAtStart() : NewItemAtEnd(); //用的每个item的size想加，所以可以解决自适应问题
                if(size <= 0) break;
                sizeFilled += size;
            }
        ...
        {            
            RectTransform nextItem = prefabSource.GetObject().transform as RectTransform; //待修改，要区分是初始创建还是回收的。
            
            if(m_uiType != null && nextItem.GetComponent(m_uiType) == null)
            {
                nextItem.gameObject.AddComponent(m_uiType);
            }
            nextItem.transform.SetParent(content, false);
            nextItem.gameObject.SetActive(true);

            var item = nextItem.GetComponent<LoopItem>();
            if(item != null)
            {
                item.ItemIndex = itemIdx;
                item.InitChildNodes(); //只有在初始时才绑定，待修改
                item.UpdateItem(itemIdx);
            }
